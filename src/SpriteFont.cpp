...
SpriteFont::SpriteFont(std::string name, std::string FileName)
{
	// <Load associate texture into TextureManager>
	Name = name;
	TextureManager::Load(name, FileName+extension);

	// Parse associated XML file
	//XMLtoCollection(path+".xml");
	XMLtoCollection(path + FileName+".xml"); // we are saving filename 
}
	...
	while (S_OK == reader->Read(&nodeType))
	{
		...
		case XmlNodeType::XmlNodeType_EndElement:
		{
			...
			if (wcscmp(stringValue, L"character") == 0)
			{
				...
				Rect* newRect = new Rect(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h));

				ImageManager::Load(Name + std::to_string(key), TextureManager::Get(Name), newRect);
				//ImageManager::Load(Name + std::to_string(key), TextureManager::Get(Name), new Rect(static_cast<float>(x) + x / 2.0f, static_cast<float>(y), static_cast<float>(w) + (w / 2.0f), static_cast<float>(h)));
				
				SESprite* newSprite = new SESprite(Name + std::to_string(key));
				//newSprite->SetCenter(-w, -h); // set to upper left corner (double check this)
				//newSprite->SetCenter( -w / 2.0f, -h / 2.0f);

				std::pair <Key, SESprite*> newItem = { key, newSprite };
				fontmap.insert(newItem);
			}
...
SESprite* SpriteFont::GetGlyph(char c) const
{
	
	assert(fontmap.find((int)c) != fontmap.end());

	return fontmap.find((int)c)->second;
